<div x-data="{
    ...
    syncFiles() {
        window.dispatchEvent(
            new CustomEvent('documents-selected', {
                detail: {
                    files: this.files,
                },
            })
    ...
    <div @drop.prevent="handleDrop($event)" @dragover.prevent="isDragging = true" @dragleave.prevent="isDragging = false"
        ...
        <input x-ref="fileInput" type="file" name="{{ $multiple ? $name . '[]' : $name }}" accept="{{ $accept }}"
            @if ($multiple) multiple @endif
            @change="
                handleFiles(Array.from($event.target.files));
                $event.target.value = '';
            "
            class="hidden" @click.stop>

        <div class="flex flex-col items-center">
            <div
                class="mb-5.5 flex h-17 w-17 items-center justify-center rounded-full bg-gray-200 text-gray-700 dark:bg-gray-800 dark:text-gray-400">
    ...
    <div x-show="errors.length > 0"
        class="mt-4 rounded-lg border border-error-500/20 bg-error-50 p-3 dark:bg-error-500/10" x-cloak>
    ...
    <div x-show="files.length > 0" class="mt-4 rounded-xl border border-gray-200 dark:border-gray-800" x-cloak>
        <div class="border-b border-gray-200 px-4 py-3 dark:border-gray-800">
            <h5 class="text-theme-sm font-semibold text-gray-700 dark:text-gray-300">
                Selected Documents
            </h5>
        </div>
            ...
            <template x-for="(file, index) in files" :key="`${file.name}-${index}`">
                <li class="flex items-center justify-between gap-3 px-4 py-3">
                    <div class="flex min-w-0 items-center gap-3">
                        <span
                            class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gray-100 dark:bg-white/5">
                        ...
                        <div class="min-w-0">
                            <p class="truncate text-theme-sm font-medium text-gray-800 dark:text-white/90"
                                x-text="file.name"></p>
                            <p class="text-theme-xs text-gray-500 dark:text-gray-400" x-text="formatSize(file.size)">
                            </p>
                        </div>
                    </div>

                    <x-ui.button x-on:click.stop="removeFile(index)" type="button" variant="danger" size="none"
                        className="flex h-8 w-8 shrink-0" aria-label="Remove file">
